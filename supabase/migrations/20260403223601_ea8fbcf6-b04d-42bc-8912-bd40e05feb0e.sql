...
-- Add rental-specific fields to db_properties
ALTER TABLE public.db_properties
  ADD COLUMN IF NOT EXISTS rental_price numeric DEFAULT 0,
...
CREATE FUNCTION public.get_property_by_slug(_slug text)
 RETURNS TABLE(
   ...
   is_highlight boolean, broker_id uuid, slug text, latitude numeric, 
   longitude numeric, virtual_tour_url text,
   rental_price numeric, condominium_fee numeric, iptu numeric,
   min_contract_months integer, accepts_pets boolean, furnished boolean,
   available_from date
 )
 LANGUAGE sql
